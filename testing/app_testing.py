...
import testing_functions as hf # helper_functions
import datetime
import csv

from utils_testing import DictToObject, stream_message, ask_to_continue, process_thread_message, EventHandler
...
async def run_conversation(message_from_ui: cl.Message):
    ...
    # Create the run - without streaming
    ...
    async with client.beta.threads.runs.stream(
        ...
        event_handler=EventHandler(),
            ...
            if hasattr(stream, '__aiter__'):
                print("Stream is an async iterator")
                async for token in stream:
                    print(token)
            elif hasattr(stream, 'read'):
                print("Stream has a read method")
                async for token in stream.read():
                    print(token)
            else:
                print("Stream is neither an async iterator nor does it have a read method")
                        ...
                        if not tool_call.id in message_references:
                            ...
                            if function_name == "get_pf_data_timeline":
                                ...
                                if parsed_output is not None:
                                    ...
                                    summary = hf.story_completion(pr.one_five_degree_prompt, units, parsed_output[parsed_output.name.str.contains('Average')])
                                    ...
                                    scenario_prompts = [' realistic, high contrast', ' stark, extreme, devastation']
                                    inpainting_keywords = ''

                                    for i in range(len(warming_scenario)):
                                        print(f"RUN STATUS: {run.status} from timeline scene # {i}")

                                        parsed_output = at.get_pf_data_timeline(address, country, warming_scenario[i], units)
                                        
                                        # filterting results to talk about change from baseline 
                                        ...
                                        #img_content, image_bytes_new = hf.get_image_response_SDXL(prompt=str(hf.summarizer(output) + scenario_prompts[i]), image_path = original_img_content, filtered_keywords=inpainting_keywords)
                                        with open('feedback_logs/73ee4d67-4857-47ec-b835-5b1cfb570b20.png', 'rb') as file:
                                           img_content = file.read()
                                        #img = cl.Image(content=image_bytes_new, name="image1", display="inline", size="large")
                                        print('\n generating image, complete')
                                        image_message_to_assistant = cl.Message(author="Climate Change Assistant", content=' ', elements=[img])
                                        await image_message_to_assistant.send() 
