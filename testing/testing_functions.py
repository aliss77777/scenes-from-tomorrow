...
def story_splitter(parsed_output):
    temperature_output = parsed_output[parsed_output.mapCategory == 'heat']
    water_output = parsed_output[parsed_output.mapCategory == 'water']
    land_output = parsed_output[parsed_output.mapCategory == 'land']
...
def get_image_response_SDXL(prompt, image_path=None, filtered_keywords=None, strength=None, inference_steps=None): #i'm passing a file path to image when using inpainting; FOR NOW
    print('starting SDXL')  # Check here for prompt language tips: https://stable-diffusion-art.com/sdxl-prompts/
    # Safely print the strength value whether it is None or a float
    ...
    if image_path is None:
        ...
        print(str(strength))
        print('it passed the strength call')
        ...
        result_image = pipeline_image2image(
            prompt=modified_prompt, image=image_path, strength=strength, guidance_scale=0.0, num_inference_steps=inference_steps).images[0]  # negative_prompt="deformed faces, distorted faces, mangled hands, extra legs", 
